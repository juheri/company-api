...
const validation = (image) => {
  fs.existsSync(publicDir + image) ? fs.unlinkSync(publicDir + image) : null;
  return true;
};
...
exports.deleteImageContent = async (id) => {
  try {
    ...
    fs.existsSync(publicDir + result.image)
      ? fs.unlinkSync(publicDir + result.image)
      : null;
    return true;
  } catch (err) {
    return { error: true };
...
exports.deleteImageProduct = async (product_id) => {
  try {
    const result = await models.product_images.findAll({
      where: { product_id },
    });
    if (result.length > 0) {
      result.map((data) => {
        fs.existsSync(publicDir + data.filename)
          ? fs.unlinkSync(publicDir + data.filename)
          : null;
      });
...
exports.resize = async (path_file) => {
  try {
    const resize = await sharp(path_file)
      .resize({
        width: 100,
        height: 100,
      })
...
exports.deleteCoverPictures = async (company_id, id) => {
  try {
    ...
    fs.existsSync(publicDir + result.filename)
      ? fs.unlinkSync(publicDir + result.filename)
      : null;
    return { error: false };
  } catch (err) {
    return { error: true };
