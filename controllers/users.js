...
exports.Register = async (req, res) => {
  try {
    let { name, address, phone, email, unique_url, password, username } =
      req.body;
    phone = regex.regexPhone(phone);
    password = await secret.password(password, res);
    const validate = await validation.validate(phone, email, unique_url);
...
exports.Logo = async (req, res) => {
  try {
    ...
    await image_helper
      .DeleteImageCompany(validate.id, "logo")
      .then(async () => {
...
exports.Avatar = async (req, res) => {
  try {
    ...
    await image_helper
      .DeleteImageCompany(validate.id, "avatar")
      .then(async () => {
        await image_helper.resize(req.file.path).then(async () => {
          await models.companies.update(
