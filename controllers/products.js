...
exports.createProduct = async (req, res) => {
  try {
    ...
    tags &&
      tags.length > 0 &&
      (await tag_controller.createProduct(result.id, tags));

    return success("OK", "success", 200, res);
  } catch (err) {
    req.files.map(async (data) => {
      await image_helper.deleteImageError(data.filename);
    });
...
exports.updateProduct = async (req, res) => {
  try {
    ...
    tags &&
      tags.length > 0 &&
      (await tag_controller.updateTagsProduct(product_id, tags));
